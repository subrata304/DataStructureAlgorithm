...
public class MaxLengthOfUnsortedSubArrayToMakeCompleteArraySorted {
	...
	private static void findUnsortedSubArray(int[] arr, int length) {
		...
		// Find the element from 0 till Start which is greater than Min and change start to that position. 
		for( i = 0; i < start; i++) 
		{ 
			if(arr[i] > min) 
			{   
		...
		// FInd the element from end+1 till array length which is smaller than Max and change the end to that position.
		...
		System.out.println("The unsorted subarray which makes "
				+ "the whole array sorted are from " + start +" : " + arr[start] + " to " + end + " : " + arr[end]);
	}
